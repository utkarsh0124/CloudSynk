...
describe('Auth Frontend Tests', () => {
    beforeEach(() => {
        fetch.resetMocks();
        // shim jQuery.ajax to forward to fetch (fetch is mocked by jest-fetch-mock)
        $.ajax = function(options) {
            ...
            fetch(options.url, fetchOptions).then(async res => {
                if (res.ok) {
                    if (options.success) {
    ...
    test('Successful logout form submission', async () => {
        ...
        // actually submit the logout form to trigger the handler
        logoutForm.submit();
        await new Promise(resolve => setTimeout(resolve, 0));
        expect(submitted).toBe(true);
