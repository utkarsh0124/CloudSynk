...
class SignupAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Check if email already exists in User or UserInfo models (case-insensitive)
        if User.objects.filter(email__iexact=email).exists() or UserInfo.objects.filter(email_id__iexact=email).exists():
        ...
        try:
            send_otp_email(from_email, subject, message)
...
class SignUpOTPVerifyAPIView(APIView):
    permission_classes = [permissions.AllowAny]
...
class ResendSignUpOTPAPIView(APIView):
    ...
    def post(self, request):
        ...
        subject = 'Your CloudSynk OTP Verification Code'
        message = f"""
                    Use the following OTP to verify your account: {code} 
                    \nThis is a resend of your OTP.
                    """
        # Send OTP via SMTP utility
        try:
            from_email = settings.DEFAULT_FROM_EMAIL
